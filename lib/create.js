...
function create() {
	...
	if ( !isArray( types ) ) {
		throw new TypeError( 'invalid input arguments. Types argument must be a string array. Value: `' + types + '`.' );
	}
	num = types.length;
	for ( i = 0; i < num; i++ ) {
		if ( types[ i ] === 'matrix' ) {
	...
	if ( n === void 0 ) {
		throw new TypeError( 'invalid input argument. At least one input argument must be a matrix.' );
	}
