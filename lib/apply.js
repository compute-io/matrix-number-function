...
* @param {Matrix|Number} value - input matrices and/or numbers
...
function apply() {
	...
	i = nargs - 1;
	if ( !isMatrixLike( args[ i ] ) && !isNumber( args[ i ] ) ) {
		nargs -= 1;
	...
	if ( M === void 0 ) {
		throw new TypeError( 'invalid input arguments. At least one argument must be a matrix.' );
	}
	if ( opts.out ) {
		out = args.shift();
		nargs -= 1;
